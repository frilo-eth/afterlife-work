...
const AskUserQuestions = forwardRef<HTMLDivElement, AskUserQuestionsProps>(
  function AskUserQuestions(
    ...
    // Detect the platform so Skip’s ⌘/⌃ hint shows the right modifier.
    // Computed in a lazy initializer (guarded for SSR, where `navigator`
    // doesn't exist) rather than an effect — an effect resolves a frame late.
    ...
    // Plain Enter commits the primary action (Continue / freeText submit).
    // ⌘/⌃+Enter is the secondary action: Skip, when the question allows it.
    const handleRootKey = (e: ReactKeyboardEvent<HTMLDivElement>) => {
      if (e.key !== 'Enter') return
      const mod = isMac ? e.metaKey : e.ctrlKey

      if (mod) {
        if (!(isSkippable && total > 1)) return
        e.preventDefault()
        handleSkip()
        return
      }

      if (e.shiftKey) return // Shift+Enter = newline inside text fields
      ...
      <div
        ...
        onFocus={(e) => {
          ...
          if (indexAttr != null) {
            ...
            setActiveIndex(idx)
            setFocusedIndex(
              !pointerFocusRedirect && (e.target as HTMLElement).matches(':focus-visible')
                ? idx
                : null,
            )
          }
          ...
          >
            <span className="inline-grid w-full">
              <textarea
                ...
                onKeyDown={(e) => {
                  // Primary: plain Enter submits Other. Secondary: ⌘/⌃+Enter
                  // skips via the root handler. Shift+Enter inserts a newline.
                  // In multi-select, plain Enter stays a newline; Continue is
                  // still available via the primary button / root Enter.
                  if (e.key !== 'Enter') return
      ...
      >
        ...
        <Field.Root invalid={freeTextError !== null} className="contents">
                ...
                {isFreeText ? (
                  ...
                  <div
                    ...
                    className={cn(
                      ...
                      isFreeTextMultiline ? 'min-h-[76px]' : compact ? 'min-h-8' : 'min-h-10',
                      shape.bg,
                    ...
                    <Field.Control
                      ...
                      render={
                        <textarea
                          ...
                          aria-labelledby={`${reactId}-${qId}-title`}
                          onKeyDown={(e) => {
                            // Primary: plain Enter submits. Secondary: ⌘/⌃+Enter
                            // skips (root handler). Shift+Enter inserts a newline.
                            if (e.key !== 'Enter') return
                            if (e.shiftKey || e.metaKey || e.ctrlKey) return
                            e.preventDefault()
                            handleOtherSubmit()
          ...
          {/* Footer — outside the morphing region, so the animating height never
              ...
              in lockstep. */}
          {showFooter && (
            <div className={cn('pt-1', compact ? 'px-3.5 sm:px-4 pb-1.5' : 'px-4 sm:px-5 pb-2')}>
                      ...
                      >
                        ...
                        >
                          <span className="inline-flex items-center gap-1.5">
                            ...
                            {/* Primary shortcut: plain Enter. Desktop-only hint. */}
                            <span className="hidden sm:contents">
