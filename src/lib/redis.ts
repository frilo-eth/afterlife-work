...
const CACHE_WARM_UP_INTERVAL = 240; // 4 minutes in seconds
...
export async function getCacheStats(): Promise<{
  ...
  try {
    ...
    const hits = Number.parseInt(info.match(/keyspace_hits:(\d+)/)?.[1] || '0');
    const misses = Number.parseInt(info.match(/keyspace_misses:(\d+)/)?.[1] || '0');
...
export async function warmUpCache(logos: Logo[] | LogoWithDetails[]): Promise<void> {
  try {
    ...
    await Promise.all([
      ...
      ...logos.map(logo => 
        setCacheWithTTL(CACHE_KEYS.LOGO_BY_ID(logo.id), logo)
      ),
    ]);
  } catch (error) {
    if (error instanceof Error && !error.message.includes('ECONNREFUSED')) {
