...
export async function seedFromCloudinary(force = false) {
  try {
    ...
    const mainLogos = cloudinaryLogos.length <= 10 
      ? cloudinaryLogos 
      : cloudinaryLogos.filter(logo => 
          !logo.public_id.includes('/gallery/') && 
          (logo.folder === 'logos' || !logo.folder)
        )
    
    // Find gallery images - those in the gallery subfolder
    // Only look for gallery images if we have enough total images
    const galleryImages = cloudinaryLogos.length > 10
      ? cloudinaryLogos.filter(image => 
          image.public_id.includes('/gallery/') || 
          image.folder === 'logos/gallery'
        )
      : []
    ...
    const galleryGroups = galleryImages.reduce((groups, image) => {
    ...
    const results = await Promise.all(mainLogos.map(async (logo) => {
      // Create pricing
      ...
      if (matchingGalleryImages.length > 0) {
        await Promise.all(matchingGalleryImages.map(galleryImage => 
        ...
        console.log(`Added ${matchingGalleryImages.length} gallery images to logo ${createdLogo.title}`)
      }
