...
async function uploadToCloudinary(file: File, publicId: string): Promise<string> {
  ...
  try {
    ...
    return new Promise((resolve, reject) => {
      ...
      enhancedLog(`📋 [CLOUDINARY] [${fileId}] Upload options: ${JSON.stringify(uploadOptions, null, 2)}`, 'info');
      ...
      try {
        const uploadStream = cloudinary.uploader.upload_stream(
          ...
          (error, result) => {
            ...
            if (error) {
              enhancedLog(`❌ [CLOUDINARY] [${fileId}] Upload error after ${uploadTime}ms: ${JSON.stringify(error)}`, 'error');
              reject(new Error(`Cloudinary upload failed: ${error.message}`));
              return;
            }
            
            if (!result) {
              enhancedLog(`❌ [CLOUDINARY] [${fileId}] No result from Cloudinary after ${uploadTime}ms`, 'error');
              reject(new Error('No result from Cloudinary'));
              return;
            }
            
            enhancedLog(`✅ [CLOUDINARY] [${fileId}] Upload successful in ${uploadTime}ms: ${JSON.stringify({
        ...
        enhancedLog(`⏳ [CLOUDINARY] [${fileId}] Sending buffer (${buffer.length} bytes) to upload stream...`, 'info');
        ...
        clearTimeout(uploadTimeout);
        enhancedLog(`❌ [CLOUDINARY] [${fileId}] Error creating upload stream: ${streamError}`, 'error');
        reject(new Error(`Failed to create upload stream: ${streamError instanceof Error ? streamError.message : 'Unknown error'}`));
      }
    });
  } catch (error) {
...
export async function PATCH(
  ...
  await enhancedLog(`📋 ${logPrefix} Request headers: ${JSON.stringify(requestHeaders, null, 2)}`, 'info');
  
  // Log URL with query parameters
  const url = new URL(request.url)
  await enhancedLog(`🔗 ${logPrefix} Request URL: ${url.toString()}`, 'info');
  await enhancedLog(`🔍 ${logPrefix} Query parameters: ${JSON.stringify(Object.fromEntries(url.searchParams.entries()), null, 2)}`, 'info');
  ...
  try {
    ...
    if (!contentType.includes('multipart/form-data')) {
      console.error(`❌ ${logPrefix} Invalid content type: ${contentType}. Expected multipart/form-data`);
    ...
    try {
      ...
      throw new Error(`Failed to parse form data: ${formDataError instanceof Error ? formDataError.message : 'Unknown error'}`);
    }
    ...
    if (clientTimestamp) {
      console.log(`🕒 ${logPrefix} Client timestamp: ${clientTimestamp}, server time difference: ${Date.now() - Number(clientTimestamp)}ms`);
    }
    ...
    for (const [key, value] of formData.entries()) {
      ...
      if (value instanceof File) {
        ...
        console.log(`📄 ${logPrefix} Form data ${key}:`, `File (${value.name}, size: ${value.size} bytes, type: ${value.type}, lastModified: ${new Date(value.lastModified).toISOString()})`)
        ...
        console.log(`📄 ${logPrefix} Form data ${key}:`, typeof value === 'string' && value.length > 100 
    ...
    console.log(`📊 ${logPrefix} Form data summary: ${fileCount} files, ${galleryImagesCount} gallery images`)
    ...
    console.log(`✅ ${logPrefix} Found logo: ${existingLogo.title} with ${existingLogo.gallery.length} gallery images`)
    ...
    for (let i = 0; i < galleryImages.length; i++) {
      ...
      console.log(`🖼️ ${logPrefix} Uploading gallery image ${i+1}/${galleryImages.length}: ${file.name}, size: ${file.size}, type: ${file.type}, lastModified: ${new Date(file.lastModified).toISOString()}`)
      
      try {
        const publicId = `gallery_${updateData.title.toLowerCase().replace(/\s+/g, '-')}_${Date.now()}_${i}`;
        console.log(`🔖 ${logPrefix} Generated Cloudinary public ID for gallery image ${i+1}:`, publicId)
    ...
    console.log(`📊 ${logPrefix} Successfully uploaded ${newGalleryImages.length} out of ${galleryImages.length} gallery images`)
    ...
    console.log(`🗑️ ${logPrefix} Processing ${deletedGalleryIds.length} deleted gallery images:`, deletedGalleryIds)
      ...
      try {
        ...
        for (let i = 0; i < galleryItemsToDelete.length; i++) {
          ...
          try {
            ...
            console.log(`🧹 ${logPrefix} Attempting to delete Cloudinary image ${i+1}/${galleryItemsToDelete.length} with public_id: ${publicId}`)
            
            // Delete from Cloudinary
            await new Promise((resolve, reject) => {
              cloudinary.uploader.destroy(publicId, (error, result) => {
                if (error) {
                  console.error(`❌ ${logPrefix} Failed to delete Cloudinary image ${publicId}:`, error)
                  reject(error)
                } else {
            ...
            console.error(`❌ ${logPrefix} Failed to delete Cloudinary image for gallery item ${item.id}:`, error)
