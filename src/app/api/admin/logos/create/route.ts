...
const LogoCreateSchema = z.object({
  ...
  cloudinaryName: z
    ...
    .regex(
      /^[a-z0-9-]+$/,
      'Cloudinary name can only contain lowercase letters, numbers, and hyphens',
    ),
  description: z
    .string()
...
async function uploadToCloudinary(file: File, publicId: string): Promise<string> {
  ...
  try {
    ...
    return new Promise((resolve, reject) => {
      ...
      const uploadStream = cloudinary.uploader.upload_stream(
        ...
        async (error, result) => {
          if (error) {
            await enhancedLog(
              `❌ [CLOUDINARY] [${fileId}] Upload failed: ${error.message}`,
              'error',
            )
          ...
          if (!result) {
            await enhancedLog(
              `❌ [CLOUDINARY] [${fileId}] Upload failed: No result returned`,
              'error',
            )
          ...
          await enhancedLog(
            `✅ [CLOUDINARY] [${fileId}] Upload successful in ${uploadTime}ms: ${result.secure_url}`,
            'info',
          )
    ...
    await enhancedLog(
      `❌ [CLOUDINARY] [${fileId}] Error during upload process: ${error instanceof Error ? error.message : String(error)}`,
      'error',
    )
    throw error
  }
...
export async function POST(request: Request) {
  ...
  try {
    ...
    await enhancedLog(
      `📦 [${requestId}] Parsed form data with ${formData.entries.length} entries`,
      'info',
    )
    ...
    try {
      ...
      if (error instanceof z.ZodError) {
        await enhancedLog(
          `❌ [${requestId}] Validation error: ${JSON.stringify(error.errors)}`,
          'error',
        )
    ...
    await enhancedLog(
      `📊 [${requestId}] Processing logo creation with ${galleryFiles.length} gallery images`,
      'info',
    )
    ...
    try {
      ...
      await enhancedLog(
        `❌ [${requestId}] Failed to upload main image: ${error instanceof Error ? error.message : String(error)}`,
        'error',
      )
    ...
    const galleryUploads: Promise<LogoGalleryCreate>[] = galleryFiles.map(async (file, index) => {
      ...
      try {
        ...
        await enhancedLog(
          `✅ [${requestId}] Gallery image ${index + 1} uploaded: ${imageUrl}`,
          'info',
        )
        ...
        await enhancedLog(
          `❌ [${requestId}] Failed to upload gallery image ${index + 1}: ${error instanceof Error ? error.message : String(error)}`,
          'error',
        )
    ...
    try {
      ...
      await enhancedLog(
        `❌ [${requestId}] Failed to upload all gallery images: ${error instanceof Error ? error.message : String(error)}`,
        'error',
      )
    ...
    try {
      ...
      await enhancedLog(
        `❌ [${requestId}] Failed to create logo in database: ${error instanceof Error ? error.message : String(error)}`,
        'error',
      )
