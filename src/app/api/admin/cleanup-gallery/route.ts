...
export async function POST(request: Request) {
  ...
  try {
    ...
    const galleryPatternLogos = allLogos.filter(
      (logo) =>
        logo.images[0].includes('/logos/gallery/') || logo.thumbnail.includes('/logos/gallery/'),
    )
    ...
    const results = await Promise.all(
      galleryPatternLogos.map(async (galleryLogo) => {
        // Find a potential parent logo
        const possibleMainLogos = allLogos.filter(
          (mainLogo) =>
            ...
            mainLogo.title
              .replace(' ', '')
              .toLowerCase()
              .includes(galleryLogo.title.replace(' ', '').toLowerCase()),
        )
